...
DrawExplosion::DrawExplosion(SDL2pp::Renderer &renderer)
    : renderer(renderer),
      texture(std::make_shared<SDL2pp::Texture>(
          renderer,
          SDL2pp::Surface("../client/assets/sprites/weapons/explosion.png"))),
      sound(std::make_shared<SDL2pp::Chunk>(
          "../client/assets/audio/explode.wav")),
      explosionClip() {
  for (int i = 0; i < 6; i++) {
...
void DrawExplosion::render(const ExplosionDTO &explosion) {
  SDL2pp::Rect dest(explosion.get_position_x() - 32,
                    explosion.get_position_y() - 32, 64, 64);
