...
AnimationMovement::AnimationMovement()
    : currentType(State::BLANK), frameCount(0), frameDuration(1),
      looping(true) {}
...
int AnimationMovement::getCurrentFrame() {
  return looping ? (frameCount / frameDuration % frameDuration)
                 : std::min(frameCount / frameDuration, frameDuration - 1);
}

void AnimationMovement::update(int frame) { frameCount += frame; }
