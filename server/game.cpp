...
Game::Game(int id, std::string name, int max_players)
    : id(id), name(name), commands(), notifier(&commands), running(true),
      next_player_id(0), actual_players(0), max_players(max_players),
      started(false), gameMap(), players() {}

void Game::add(Socket &&socket) {
  notifier.notify(std::make_shared<NewPlayer>(
      get_actual_players(), get_max_players(), get_players_data()));
...
void Game::run() {
  try {
    ...
    // Este sleep es para que el game no cierre la conexión y le llegue el
    // GameOver a los clientes
...
void Game::checkFinishGame() {
  if (gameMap.check_finished()) {
    ...
    std::shared_ptr<Event> event = std::make_shared<GameOver>(
        PlayerData(winner_id, players[winner_id]), 10);
    notify_event(event);
    running = false;
  }
}
...
void Game::checkNewRound() {
  if (gameMap.pauseForScores()) {
    ...
    std::sort(score_name_pairs.begin(), score_name_pairs.end(),
              std::greater<>());
...
void Game::notify_state() {
  std::shared_ptr<Event> event = std::make_shared<Broadcast>(
      gameMap.getState(), gameMap.getBulletsState(), gameMap.getCratesState(),
      gameMap.getItemSpawnsState(), gameMap.getExplosionsState(),
      gameMap.getThrowablesState());
  notify_event(event);
}
...
bool Game::is_running() { return running && !notifier.any_connected(); }
