...
int32_t Encoder::decode_int32(std::vector<int8_t> &data) {
  if (data.size() < sizeof(int32_t)) {
    throw std::runtime_error(
        "No hay suficientes bytes para decodificar el entero de 32 bits.");
  }
  int32_t value;
...
int16_t Encoder::decode_int16(std::vector<int8_t> &data) {
  if (data.size() < sizeof(int16_t)) {
    throw std::runtime_error(
        "No hay suficientes bytes para decodificar el entero de 16 bits.");
  }
  int16_t value;
...
int8_t Encoder::decode_int8(std::vector<int8_t> &data) {
  if (data.size() < sizeof(int8_t)) {
    throw std::runtime_error(
        "No hay suficientes bytes para decodificar el entero de 8 bits.");
  }
  int8_t value;
...
int Encoder::decode_len(std::vector<int8_t> &data) { return decode_int8(data); }
...
size_t Encoder::encode_id(int id, void *data) { return encode_int8(id, data); }

int Encoder::decode_id(std::vector<int8_t> &data) { return decode_int8(data); }
