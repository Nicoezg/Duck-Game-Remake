...
#define WEAPONS_SIZE                                                           \
  (WEAPON_ID_SIZE + COORDINATE_SIZE * 2 + IS_SHOOTING_SIZE + HAS_AMMO_SIZE)

#define PLAYER_DATA_SIZE (PLAYERS_DATA_ID_SIZE + LEN_SIZE)
#define GAME_ROOM_SIZE                                                         \
  (MAX_PLAYER_SIZE + ACTUAL_PLAYER_SIZE + GAME_CODE_SIZE + LEN_SIZE)
...
#define READ_BULLET_SIZE                                                       \
  (COORDINATE_SIZE * 2 + BULLET_ID_SIZE + ANGLE_SIZE + BULLET_IS_RIGHT_SIZE)
...
#define READ_THROWABLE_SIZE                                                    \
  (COORDINATE_SIZE * 2 + THROWABLE_ID_SIZE + sizeof(uint8_t) + ANGLE_SIZE +    \
   sizeof(uint8_t))
...
#define READ_TILE_SIZE (COORDINATE_SIZE * 4 + TILE_ID_SIZE)
...
#define SEND_GAME_OVER_SIZE                                                    \
  (READ_GAME_OVER_SIZE + PLAYER_DATA_SIZE + EVENT_TYPE_SIZE)
...
public:
  ...
  void add_players(const Event &event, std::vector<int8_t> &data,
                   size_t &offset);
  ...
  void add_platforms(const Event &event, std::vector<int8_t> &data,
                     size_t &offset);

  void add_bullets(const Event &event, std::vector<int8_t> &data,
                   size_t &offset);

  void add_crates(const Event &, std::vector<int8_t> &data, size_t &offset);

  void add_item_spawns(const Event &, std::vector<int8_t> &data,
                       size_t &offset);
  ...
  void add_game_room(const GameRoom &game_room, std::vector<int8_t> &data,
                     size_t &offset);
  ...
  void add_players_data(std::vector<int8_t> &data, size_t &offset,
  ...
  void add_player_data(std::vector<int8_t> &data, size_t &offset,
  ...
  void add_scores(std::vector<int8_t> &data, size_t &offset,
                  const std::list<int> &scores);

  void add_names(std::vector<int8_t> &data, size_t &offset,
