...
#define READ_JOIN_GAME_SIZE                                                    \
  (2 * PLAYERS_ID_SIZE + CONNECTED_SIZE + ACTUAL_PLAYER_SIZE + MAX_PLAYER_SIZE)
...
#define READ_PLAYER_SIZE                                                       \
  (2 * PLAYER_COORDINATE + PLAYERS_ID_SIZE + PLAYER_STATE_SIZE +               \
...
public:
  ...
  void add_players(const std::shared_ptr<Event> &event,
  ...
  void add_platforms(const std::shared_ptr<Event> &event,
                     std::vector<int8_t> &data, size_t &offset);

  void add_bullets(const std::shared_ptr<Event> &event,
                   std::vector<int8_t> &data, size_t &offset);
