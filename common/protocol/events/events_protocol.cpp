...
size_t EventsProtocol::get_size_to_send(const Event &event, size_t start) {
  int game_room_size =
      GAME_ROOM_SIZE + event.get_game_room().get_game_name().size();
...
void EventsProtocol::send_broadcast(const Event &event) {
  std::vector<int8_t> data(
      ...
      SEND_EXPLOSION_SIZE * event.get_explosions().size() + EVENT_TYPE_SIZE +
      event.get_crates().size() * SEND_CRATE_SIZE +
      event.get_item_spawns().size() * SEND_ITEM_SPAWN_SIZE +
...
void EventsProtocol::add_bullets(const Event &event, std::vector<int8_t> &data,
                                 size_t &offset) {
...
void EventsProtocol::add_crates(const Event &event, std::vector<int8_t> &data,
                                size_t &offset) {
...
void EventsProtocol::add_item_spawns(const Event &event,
  ...
  for (const auto &item_spawn : event.get_item_spawns()) {
    ...
    offset +=
        encoder.encode_coordinate(item_spawn.get_position_x(), &data[offset]);
    offset +=
        encoder.encode_coordinate(item_spawn.get_position_y(), &data[offset]);
  }
}
...
void EventsProtocol::add_explosions(const Event &event,
  ...
  for (const auto &explosion : event.get_explosions()) {
    offset +=
        encoder.encode_coordinate(explosion.get_position_x(), &data[offset]);
    offset +=
        encoder.encode_coordinate(explosion.get_position_y(), &data[offset]);
    offset +=
        encoder.encode_id(explosion.get_current_duration(), &data[offset]);
  }
}
...
void EventsProtocol::add_throwables(const Event &event,
  ...
  for (const auto &throwable : event.get_throwables()) {
    ...
    offset +=
        encoder.encode_coordinate(throwable.get_position_x(), &data[offset]);
    offset +=
        encoder.encode_coordinate(throwable.get_position_y(), &data[offset]);
...
void EventsProtocol::add_players(const Event &event, std::vector<int8_t> &data,
                                 size_t &offset) {
...
void EventsProtocol::send_new_player(const Event &event) {
  ...
  offset += encoder.encode_event_type(event.get_type(), &data[offset]);
  offset +=
      encoder.encode_actual_players(event.get_actual_players(), &data[offset]);
...
void EventsProtocol::send_map(const Event &event) {
  ...
  offset +=
      encoder.encode_background_id(event.get_background_id(), &data[offset]);
...
void EventsProtocol::add_game_room(const GameRoom &game_room,
                                   std::vector<int8_t> &data, size_t &offset) {
  offset += encoder.encode_game_code(game_room.get_game_code(), &data[offset]);
  offset += encoder.encode_actual_players(game_room.get_actual_players(),
                                          &data[offset]);
  offset +=
      encoder.encode_max_players(game_room.get_max_players(), &data[offset]);
  add_name(game_room.get_game_name(), data, offset);
}

void EventsProtocol::add_name(const std::string &name,
                              std::vector<int8_t> &data, size_t &offset) {
...
void EventsProtocol::add_players_data(
    std::vector<int8_t> &data, size_t &offset,
    const std::list<PlayerData> &players_data) {
...
void EventsProtocol::add_player_data(std::vector<int8_t> &data, size_t &offset,
...
void EventsProtocol::add_names(std::vector<int8_t> &data, size_t &offset,
...
void EventsProtocol::add_scores(std::vector<int8_t> &data, size_t &offset,
                                const std::list<int> &scores) {
