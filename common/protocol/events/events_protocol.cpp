...
std::shared_ptr<Event> EventsProtocol::read_broadcast() {
  ...
  for (int i = 0; i < bullets_len; i++) {
    ...
    bool direction = encoder.decode_is_right(bullets_data);
...
void EventsProtocol::add_bullets(const std::shared_ptr<Event> &event,
  ...
  for (const auto &bullet : event->get_bullets()) {
    ...
    offset += encoder.encode_is_right(bullet.get_direction(), &data[offset]);
  }
}

void EventsProtocol::add_crates(const std::shared_ptr<Event> &event,
                                std::vector<int8_t> &data, size_t &offset) {
...
void EventsProtocol::add_players(const std::shared_ptr<Event> &event,
                                 std::vector<int8_t> &data, size_t &offset) {
...
void EventsProtocol::send_refresh(const std::shared_ptr<Event> &event) {
  std::vector<int8_t> data(
      LEN_SIZE + event->get_games().size() * GAME_ROOM_SIZE + EVENT_TYPE_SIZE);
  size_t offset = 0;
  offset += encoder.encode_event_type(event->get_type(), &data[offset]);
  ...
  for (const auto &game : event->get_games()) {
    ...
    offset +=
        encoder.encode_actual_players(game.get_actual_players(), &data[offset]);
...
void EventsProtocol::add_platforms(const std::shared_ptr<Event> &event,
                                   std::vector<int8_t> &data, size_t &offset) {
