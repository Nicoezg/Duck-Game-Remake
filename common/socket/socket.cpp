...
Socket::Socket(const char *hostname, const char *servname) {
  ...
  while (resolver.has_next()) {
     ...
     * */
    s = connect(sk, addr->ai_addr, addr->ai_addrlen);
    if (s == -1) {
      continue;
    }
  ...
  /*
   * Si el `sk` es -1 es por q (1) no entramos nunca al for-loop
   * o (2) la ultima llamada `socket` fallo y retorno -1.
   * En ambos casos no debemos cerrar ningun socket.
   *
   * Si en cambio `sk` es distinto de -1 significa q tenemos
   * un socket abierto.
   * */
  if (sk != -1)
    ::close(sk);
...
Socket::Socket(const char *servname) {
  ...
  while (resolver.has_next()) {
     ...
     *
     * Con SO_REUSEADDR le decimos al OS q "esta tod bien" si el
     * puerto aun esta en WAIT, y q nos debe dejar hacer un bind.
     *
     * De ahí el nombre "reuse address" o "SO_REUSEADDR".
     **/
    int optval = 1;
    s = setsockopt(sk, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval));
    if (s == -1) {
      continue;
    }
     ...
     * */
    s = bind(sk, addr->ai_addr, addr->ai_addrlen);
    if (s == -1) {
      continue;
    }
...
int Socket::sendsome(void *data, unsigned int sz, bool *was_closed) {
  chk_skt_or_fail();
  *was_closed = false;
  /*
   ...
   * (ver más abajo).
   * */
  int s = send(this->skt, reinterpret_cast<char *>(data), sz, MSG_NOSIGNAL);
  if (s == -1) {
    /*
  ...
  *was_closed = false;
  ...
  while (received < sz) {
    ...
    if (s <= 0) {
      ...
      /*
       * OK, recibimos algo pero no necesariamente tod lo que
...
int Socket::sendall(void *data, unsigned int sz, bool *was_closed) {
  unsigned int sent = 0;
  *was_closed = false;

  while (sent < sz) {
    int s =
        sendsome(reinterpret_cast<char *>(data) + sent, sz - sent, was_closed);
...
Socket Socket::accept() {
   ...
   *
   * En tod momento podemos seguir usando *nuestro* file descriptor
