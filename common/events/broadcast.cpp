...
Broadcast::Broadcast(std::list<PlayerDTO> &&players, std::list<BulletDTO> &&bullets, std::list<CrateDTO> &&crates,
                     std::list<ItemSpawnDTO> &&item_spawns, std::list<ExplosionDTO> &&explosions, std::list<ThrowableDTO> &&throwables)
        : Event(BROADCAST), players(std::move(players)), bullets(std::move(bullets)), crates(std::move(crates)),
          item_spawns(std::move(item_spawns)), explosions(std::move(explosions)), throwables(std::move(throwables)) {}
