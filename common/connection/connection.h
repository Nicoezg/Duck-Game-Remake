...
public:
    Connection(Socket &&socket, Encoder encoder, Queue<U> *commands, Queue<V> *states) :
            socket(std::move(socket)), protocol(&this->socket, encoder),
            reader(&this->protocol, commands), sender(&this->protocol, states), was_closed(false) {}
    ...
    bool is_open() {
        return reader.is_open() && sender.is_open();
    }
